...
func (c *CloudConfig) NewMasterTemplate(customObject kvmtpr.CustomObject, certs certificatetpr.AssetsBundle, node clustertprspec.Node) (string, error) {
	...
	if customObject.Spec.Cluster.K8sCloudConfig.Version == "" {
		customObject.Spec.Cluster.K8sCloudConfig.Version = string(cloudconfig.V_0_1_0)
	}

	var template string

	switch customObject.Spec.Cluster.K8sCloudConfig.Version {
	...
	default:
		return "", microerror.Maskf(notFoundError, "k8scloudconfig version '%s'", customObject.Spec.Cluster.K8sCloudConfig.Version)
	}
