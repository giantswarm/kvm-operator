...
func (r *Resource) ApplyUpdateChange(ctx context.Context, obj, updateChange interface{}) error {
	...
	if len(configMapsToUpdate) != 0 {
		r.logger.LogWithCtx(ctx, "debug", "updating the config maps in the Kubernetes API")
		...
		r.logger.LogWithCtx(ctx, "debug", "updated the config maps in the Kubernetes API")
	} else {
		r.logger.LogWithCtx(ctx, "debug", "the config maps do not need to be updated in the Kubernetes API")
	}
...
func (r *Resource) newUpdateChange(ctx context.Context, obj, currentState, desiredState interface{}) (interface{}, error) {
	...
	{
		r.logger.LogWithCtx(ctx, "debug", "finding out which config maps have to be updated")
		...
		r.logger.LogWithCtx(ctx, "debug", fmt.Sprintf("found %d config maps that have to be updated", len(configMapsToUpdate)))
	}
