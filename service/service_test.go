	...
	tests := []struct {
		...
		// Test a production-like config is valid.
		{
			config: func() Config {
				...
				config.Viper.Set(config.Flag.Service.Installation.Provider.KVM.DNS.Servers, "dnsserver1,dnsserver2")
				config.Viper.Set(config.Flag.Service.Kubernetes.InCluster, "false")
				config.Viper.Set(config.Flag.Service.Guest.SSH.SSOPublicKey, "test")
